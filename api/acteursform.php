...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["acteurImage"])) {
    $targetDir = "images/";
    if (!is_dir($targetDir)) {
        mkdir($targetDir, 0777, true);
    }

    $fileName = time() . "_" . basename($_FILES["acteurImage"]["name"]); // Nom unique
    $targetFilePath = $targetDir . $fileName;
    $imageFileType = strtolower(pathinfo($targetFilePath, PATHINFO_EXTENSION));

    $allowedTypes = ["jpg", "jpeg", "png", "gif"];
    if (in_array($imageFileType, $allowedTypes)) {
        if (move_uploaded_file($_FILES["acteurImage"]["tmp_name"], $targetFilePath)) {
